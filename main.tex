...
\vspace{0.25 cm}
\normalsize
Rather than calling \mintinline{c}{malloc} for each new object, they are created in preallocated memory pools. Each pool represents a different generation; those that survive mark-and-sweep are {\it copied} from one pool into the next.
...
\vspace{0.2 cm}
The 3 generations are different doubly linked lists. Mark-and-sweep marks are in the low bits of list pointers so that garbage collection has a constant memory footprint.
...
\hfill \textcolor{blue}{\url{https://docs.julialang.org/en/v1/devdocs/object}}\end{minipage}}

\vspace{0.25 cm}
\normalsize
Small objects ($< 2$~kB) are managed in preallocated pools; large objects use \mintinline{c}{malloc}.
...
\begin{minted}{c}
        typedef struct {
            opaque metadata;    /* header with GC bits */
...
\vspace{0.2 cm}
\normalsize
Marking is depth-first and parallel; sweeping is serial. Memory is returned to the operating system on a per-page basis, once a page has zero surviving objects, freed with \mintinline{c}{madvise} on a background thread.

\vspace{0.2 cm}
Julia makes stack-versus-heap user-visible, to avoid garbage collection when possible.
\end{frame}
